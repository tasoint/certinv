...
func TestOptionalBasicAuthProtectsUIManagement(t *testing.T) {
	...
	mux.HandleFunc("/ui/hosts/suppress-all", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusSeeOther)
	})
	mux.HandleFunc("/ui/hosts/purge-all", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusSeeOther)
	})
	mux.HandleFunc("/ui/crtname/lookup", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusSeeOther)
	})
	mux.HandleFunc("/ui/crtname/add-selected", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusSeeOther)
	})
	handler := withOptionalBasicAuth(mux, config.ExporterAuth{Username: "operator", Password: "secret"})
